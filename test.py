...
def test(model: CPR, train_fns, test_fns, retrieval_result, foreground_result, resize, region_sizes, root_dir, knn, T, vis_dir=None, use_groundtruth=True):
    ...
    for image_fn in tqdm(test_fns, desc='predict test data', leave=False):
        ...
        if vis_dir is not None:
            ...
            norm_heatmap = (heatmap - min_score) / (max_score - min_score + 1e-10)  
            
            heatmap_resized = cv.resize(norm_heatmap, (defect.shape[1], defect.shape[0]))
            ...
            mask_gt_color = cv.cvtColor(mask_gt, cv.COLOR_GRAY2BGR)  
            img_up = np.hstack([defect, mask_gt_color, heatmap_color])  
            img_up = cv.resize(img_up, (256 * 3, 256))

            image_name = os.path.basename(image_fn).split('.')[0]
            save_path = os.path.join(vis_dir, anomaly_name, f'heatmap_{image_name}.png')
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            cv.imwrite(save_path, img_up)
