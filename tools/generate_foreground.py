...
@torch.no_grad()
def gen_foreground(input_path, save_path, dataset_name, model_name, layer, resize, vis):
    ...
    for sub_category in dataset_info[1]:  # object
        ...
        model.eval()
        root_dir = os.path.join(input_path, dataset_name, sub_category)
...
if __name__ == "__main__":
    ...
    # data
    parser.add_argument("-dd", "--data_dir", type=str, default="./data", help="Path to the dataset directory")
    parser.add_argument("--dataset-name", type=str, default="mvtec", choices=["mvtec", "mvtec_3d", "btad",
    ...
    mlflow.set_experiment("CPR_foreground")

    logger.add(os.path.join(args.log_path, 'runtime.log'))
    logger.info('args: \n' + pformat(vars(args)))

    with mlflow.start_run(run_name=f"{args.pretrained_model}_{args.layer}_{args.resize}"):
        ...
        gen_foreground(args.data_dir, args.log_path, args.dataset_name, args.pretrained_model, args.layer, args.resize, args.vis)
